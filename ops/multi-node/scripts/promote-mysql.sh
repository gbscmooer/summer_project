...
if [[ "${OLD_PRIMARY_FENCED:-}" != "yes" ]]; then
  ...
  echo "       Planned switch: set read_only=ON on the old primary first." >&2
  echo "       Failure switch: confirm the old primary is powered off, disconnected, or security-group isolated." >&2
  exit 1
fi
...
if [[ -n "${last_sql_err}" ]]; then
  echo "ERROR: Last_SQL_Error=${last_sql_err} (must be empty; rejected even with FORCE_PROMOTE=1)" >&2
  exit 1
fi
...
if [[ "$failure_promote" -eq 0 ]]; then
  ...
  if [[ -z "${lag}" || "${lag}" == "NULL" || "${lag}" != "0" ]]; then
    echo "ERROR: Seconds_Behind_Source=${lag:-<empty>} (planned switch requires lag = 0)" >&2
    exit 1
  fi
  echo "==> planned switch: IO/SQL=Yes, no replication errors, lag=0"
  echo "==> confirm old primary (Data-Main) is read-only: read_only=ON and super_read_only=ON"
else
  echo "==> failure switch: confirm old primary is powered off, disconnected, or security-group isolated"
  echo "==> WARN: promoting with possible RPO data loss"
fi

echo
echo "==> pre-promote fencing check (OLD_PRIMARY_FENCED=yes)"
if [[ "$failure_promote" -eq 0 ]]; then
  echo "    [planned] old primary must stay online but read-only; do not stop it or cut network"
else
  echo "    [failure] old primary must already be fenced (powered off / disconnected / security-group isolated)"
  echo "    [failure] RPO data loss is possible when IO=No"
fi
echo
